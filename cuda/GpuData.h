...
static constexpr int   TERMBITS         = 10;
static constexpr float MAXTERM          = (float)(1 << (31 - TERMBITS - 8)); // 2^(31 - 10 - 8) = 2^13 = 8192
static constexpr float TERMSCALE        = (float)(1 << TERMBITS);            // 2^10 = 1024
static constexpr float ONEOVERTERMSCALE = 1.0f / TERMSCALE;                  // 1 / 1024 = 0.000977
static constexpr float MAXREDUCE        = (float)(1 << (31 - TERMBITS - 4)); // 2^(31 - 10 - 4) = 2^17 = 131072

static constexpr float MAXENERGY        = FLT_MAX / 100.0f; // Used to cap absurd energies so placeholder energy is always skipped in sorts
static constexpr float MAXFORCE         = FLT_MAX / 100.0f; // Used to cap absurd gradients
